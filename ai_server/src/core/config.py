...
class Settings(BaseSettings):
    ...
    POSTGRES_MAX_CONNECTIONS_PER_POOL: int = 5

    TABLE_NAME_PREFIX: str = (
        "agent_server_"  # Prefix for all tables created by the application in PostgreSQL
    )

    EMBEDDING_MODEL_NAME: str | None = None

    # 安全防护配置
    ENABLE_SAFEGUARD: bool = Field(default=True, description="是否启用安全防护（提示注入检测）")
        ...
        for provider in active_keys:
            match provider:
                ...
                case Provider.DASHSCOPE:
                    if self.DEFAULT_MODEL is None:
                        self.DEFAULT_MODEL = DashScopeModelName.QWEN_TURBO
    ...
    REDIS_MAX_CONNECTIONS: int = Field(default=50, description="Maximum Redis connections in pool")

    # ==========================================
    # JWT Authentication Configuration
    # ==========================================
    JWT_SECRET_KEY: SecretStr | None = Field(
        default=None, description="Secret key for JWT token signing (required for auth)"
    )
    ...
    RATE_LIMIT_ANONYMOUS: str = Field(
        default="10/minute", description="Rate limit for anonymous requests"
    )
    RATE_LIMIT_AUTHENTICATED: str = Field(
        default="60/minute", description="Rate limit for authenticated users"
    )
    ...
    # ==========================================
    SQLALCHEMY_POOL_SIZE: int = Field(default=5, description="SQLAlchemy connection pool size")
    SQLALCHEMY_MAX_OVERFLOW: int = Field(
        default=10, description="SQLAlchemy max overflow connections"
    )

    # ==========================================
    # PostgreSQL Pool Optimization
    # ==========================================
    POSTGRES_POOL_OPEN_TIMEOUT: int = Field(
        default=30, description="Seconds to wait for connection from pool"
    )
    POSTGRES_POOL_CLOSE_TIMEOUT: int = Field(
        default=10, description="Seconds to wait when closing pool"
    )
    POSTGRES_POOL_MAX_IDLE_TIME: int = Field(
        default=300, description="Seconds before idle connection is recycled"
    )
    ...
    CELERY_RESULT_BACKEND: str = Field(
        default="redis://localhost:6379/1", description="Celery result backend URL (Redis)"
    )
    CELERY_WORKER_CONCURRENCY: int = Field(
        default=4, description="Number of concurrent Celery workers"
    )
